import asyncio
from time import time  # Импортируем time

start = time()  # Засекаем начало работы программы
...
# Узнаем количество записей. Если программа работает линейно - ожидаемое время: 51*1 сек.
print(f'Количеств записей: {len(messages)}')  # 51


async def check_message(message):
    await asyncio.sleep(1)  # "Затягиваем" время работы каждой задачи на 1 сек
    for word in banned_words:
        if word.lower() in message["message"].lower():
            task = asyncio.current_task()
            task.cancel()
            print(f"В сообщении {message['message_id']} стоп-слово: task.done(): {task.done()}")
            return
...
async def main():
    ...
    for task in tasks:
        try:
            ...
            continue
    print(f"На работу программы затрачено: {time() - start} секунд.")  # Выводим время работы программы

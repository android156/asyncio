...
async def check_message(mes):
    ...
    try:
        for word in mes['message'].split():
            if word.strip('.,').lower() in banned_words:
                await asyncio.sleep(0)
                print(f'В сообщении {mes['message_id']} стоп-слово: task.done(): {task.done()}')
                task.cancel()
                break
        await asyncio.sleep(0)
        print(f'{mes['message_id']}: {mes['message']}')
...
async def main():
    tasks = [asyncio.create_task(check_message(mes), name=mes['message_id']) for mes in message]

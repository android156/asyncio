...
def check_text(text):
    clear_text = text
    for expression in banned_words:
        if text.find(expression):
            clear_text = clear_text.replace(expression, "****")
    if clear_text != text:
        return clear_text
...
async def main():
    tasks = [asyncio.create_task(check_message(mes), name=mes['role']) for mes in message]

...
# Для вашего удобства, полный список вшит в задачу
data = [
...
# async def call_company(info):
#     if info['call_time'] > 5:
#         raise asyncio.CancelledError()
#     else:
#         print(f'Company {info["Name"]}: {info["Phone"]} дозвон успешен')
#
#
# async def main():
#     [asyncio.create_task(call_company(info)) for info in data]

# мой вариант

# async def call_company(company):
#     time = company['call_time']
#     try:
#         await asyncio.sleep(time)
#         return f'Company {company['Name']}: {company['Phone']} дозвон успешен'
#     except asyncio.CancelledError:
#         raise
#
#
#
# async def main():
#     tasks = [asyncio.create_task(call_company(company), name=company['Name']) for company in data]
#
#     done, pending = await asyncio.wait(tasks, timeout=5.1)
#
#
#     for task in done:

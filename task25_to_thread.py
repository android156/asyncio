...
async def download_and_process(article: dict):
    ...
    coro = asyncio.to_thread(find_most_common_word, article['content'])
    article['tag'] = await asyncio.create_task(coro)


async def main():
    await asyncio.gather(*[download_and_process(article) for article in articles])

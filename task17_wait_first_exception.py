...
async def check_access(agent_data):
    await asyncio.sleep(agent_data.get('Уровень секретности', 0))
    if agent_data.get("Срок доступа"):
        print(f'Участник {agent_data["Имя"]} {agent_data["Фамилия"]} имеет действующий доступ. Продолжительность доступа: {agent_data["Срок доступа"]}')
    else:
        raise ValueError(f'Ошибка доступа: У участника {agent_data["Имя"]} {agent_data["Фамилия"]} срок доступа истек или не указан.')


async def main():
    tasks = [asyncio.create_task(check_access(agent_data), name=f'{agent_data["Имя"]} {agent_data["Фамилия"]}') for agent_data in data]
